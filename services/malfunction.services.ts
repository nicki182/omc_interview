...
class MalfunctionServices implements CRUDService<MalfunctionDTO, MalfunctionWithSensor> {
    ...
    public async create(data: Malfunction): Promise<MalfunctionDTO> {
        ...
        await this.redis.set(`${this.cacheKey}${malfunction.id}`, JSON.stringify(malfunction));
    ...
    public async list(): Promise<MalfunctionDTO[]> {
        ...
        logger.info(`Retrieved ${malfunctions.length} malfunctions from the database`);
    ...
    public async update(id: number, data: Malfunction): Promise<MalfunctionDTO | null> {
